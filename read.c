...
/* modified by David Nichols for PM MOD player */
...
char *getstring (FILE *f, int len)
{
  ...
  s[MIN (len, MAX_LEN - 1)] = '\0';
   DosAllocMem((PVOID) &new, strlen(s) + 1, PAG_READ|PAG_WRITE|PAG_COMMIT);
...
void fill_sample_info (struct sample_info *info, FILE *f)
{
...
void fill_song_info (struct song_info *info, FILE *f)
{
  int i;
  int p;
...
void fill_event(struct event *e, FILE *f)
{
  int a, b, c, d;
...
void fill_pattern (struct block *pattern, FILE *f)
{
  int i, j;
...
void read_sample (struct sample_info *info, FILE *f)
{
  if (info->start)
    {
...
struct song *new_song (void)
{
  ...
  int i;
   ...
   DosAllocMem((PVOID)&new, sizeof (struct song), PAG_READ|PAG_WRITE|PAG_COMMIT);
...
struct sample_info *new_sample_info (void)
{
  struct sample_info *new;
   ...
   DosAllocMem((PVOID)&new, sizeof(struct sample_info), PAG_READ|PAG_WRITE|PAG_COMMIT);
...
struct song_info *new_song_info (void)
{
  struct song_info *new;
   ...
   DosAllocMem((PVOID)&new, sizeof(struct song_info), PAG_READ|PAG_WRITE|PAG_COMMIT);
  ...
  for (i = 0; i < 31; i++)
      ...
      if (song->samples[i])
	{
	  if (song->samples[i]->start)
	    DosFreeMem(song->samples[i]->start);
/*	    free (song->samples[i]->start); */
	  if (song->samples[i]->name)
  	    DosFreeMem(song->samples[i]->name);
/*	    free (song->samples[i]->name); */
	  DosFreeMem(song->samples[i]);
/*	  free (song->samples[i]); */
	}
    }
  if (song->info)
	...
	DosFreeMem(song->info->pblocks);
/*	free (song->info->pblocks); */
      DosFreeMem(song->info);
/*      free (song->info); */
    }
  if (song->title)
      DosFreeMem(song->title);
/*    free (song->title); */
   DosFreeMem(song);
/*  free (song); */
}
...
struct song *read_song (FILE *f, int type, int tr)
{
  ...
  DosAllocMem((PVOID)&song->info->pblocks, sizeof (struct block) * song->info->maxpat,
	PAG_READ|PAG_WRITE|PAG_COMMIT);
/* song->info->pblocks = (struct block *)malloc(sizeof(struct block) * song->info->maxpat); */
